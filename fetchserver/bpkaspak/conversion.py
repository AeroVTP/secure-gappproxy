...
def GE2OSP(point, domain):
    return ECP2OSP(point, domain)#FE2OSP(point.x(),domain) + FE2OSP(point.y(), domain)

def OS2GEP(serialized, domain):
    return OS2ECPP(serialized, domain)
    #from ellipticcurve import *
    #return Point(domain, OS2FEP(serialized[:domain.len], domain), OS2FEP(serialized[domain.len:], domain))
...
def ECP2OSP(point, domain):
    ...
    import random
    even_rand = random.SystemRandom().randrange(0,128) * 2
    if point.y() % 2 == 0:
        result += I2OSP(even_rand, 1)
    else:
        result += I2OSP(even_rand + 1, 1)
    return result
